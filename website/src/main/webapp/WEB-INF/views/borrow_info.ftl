    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#bidBtn").on("click", function () {
                var amount = pasreMoney($("#amount").val());
                if (!amount) {
                    $.messager.popup("请输入投资金额");
                    return;
                }

                if (pasreMoney($("#usableAmount").val()) < amount) {
                    $.messager.popup("投资金额已超过账户余额");
                    return;
                }
                if (pasreMoney($("#minBidAmount").val()) > amount) {
                    $.messager.popup("投资金额低于最小投标金额");
                    return;
                }
                if (pasreMoney($("#maxBidAmount").val()) < amount) {
                ...
                <tr>
                    <td colspan="2">
                        <div style="margin-bottom: 10px;" class="progress">
                            ...
                            <#else>
                                 <div style="width: ${bidRequest.persent}%"
                                      class="progress-bar progress-bar-info progress-bar-striped"></div>
                            </#if>
                        </div>
                    </td>
					...
					<#if bidRequest.bidRequestState==1>
                        <#if self>
							<br/>
							<a class="btn btn-danger btn-block" style="font-size: 18px;" href="javascript:void(0);">
                                投标中
                            </a>
                        <#elseif account?? && !self>
